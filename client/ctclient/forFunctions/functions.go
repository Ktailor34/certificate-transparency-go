...
func publicKeyToString(_ x509.PublicKeyAlgorithm, pub interface{}) (int, []byte, int) {
...
func showCriticalPrint(critical bool) string {
	if critical {
		return "critical "
	}
	return ""
...
func showAuthKeyID(cert *x509.Certificate) string {
	count, critical := OIDInExtensions(x509.OIDExtensionAuthorityKeyId, cert.Extensions)
	if count > 0 {

		criticalString := showCriticalPrint(critical)
		return criticalString + hex.EncodeToString(cert.AuthorityKeyId)
	}
	return ""
}

func showSubjectKeyID(cert *x509.Certificate) string {
	...
	if count > 0 {
		...
		return criticalString + hex.EncodeToString(cert.SubjectKeyId)
	}
	return ""
}

func showKeyUsage(cert *x509.Certificate) string {
	...
	if count > 0 {
		...
		return criticalString + keyUsageToString(cert.KeyUsage)
	}
	return ""
}

func showExtendedKeyUsage(cert *x509.Certificate) string {
	...
	if count > 0 {
		...
		//result.WriteString(fmt.Sprintf("                %v\n", usages.String()))
		return criticalString + usages.String()
	}
	return ""
}

func showBasicConstraints(cert *x509.Certificate) string {
	...
	if count > 0 {
		...
		if cert.MaxPathLen > 0 || cert.MaxPathLenZero {
			return criticalString + "pathlen " + string(cert.MaxPathLen)
...
func showSubjectAltName(cert *x509.Certificate) string {
	...
	if count > 0 {
		...
		return criticalString + buf.String()
		//result.WriteString(fmt.Sprintf("                %v\n", buf.String()))
...
func showCertPolicies(cert *x509.Certificate) string {
	...
	if count > 0 {
		...
		for _, oid := range cert.PolicyIdentifiers {
			...
			policies = policies + oid.String() + " "
		}
		return criticalString + policies
	}
	return ""
}
...
func showAuthInfoAccess(cert *x509.Certificate) (string, string) {
	...
	if count > 0 {
		...
		return issuerBuf.String(), ocspBuf.String()
	}
	return "", ""
}
...
func showCTSCT(cert *x509.Certificate) []SCT {
	...
	if count > 0 {
		...
		for _, sctData := range cert.SCTList.SCTList {
			...
			mySCT := SCT{
				...
				SignatureAlgorithm: sct.Signature.Algorithm.Signature.String(), //TODO1
				Signature:          sct.Signature.Signature,
			}

			SCT_List = append(SCT_List, mySCT)
		}
	}
	return SCT_List
}
...
func showSignature(cert *x509.Certificate) []byte {
	//result.WriteString(fmt.Sprintf("    Signature Algorithm: %v\n", cert.SignatureAlgorithm))
